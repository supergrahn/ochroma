...
- **Four UI faces coexist.** egui shell (engine editor), UiTree+Vello overlay (engine HUD), a 968-line hand-rolled ab_glyph raster HUD (`urban_horizon/src/render_gpu/hud.rs`), and a tiny-skia menu renderer (`urban_horizon/src/menu/render.rs`). Only the first two share `Tokens`.
- **Asset quality judgment is a terminal ritual.** The six-view inspection harness (`urban_horizon/src/bin/forge_pathtrace.rs` — six views per flagship asset, calibrated pixel gates printing `gates: PASS`/`gates: FAIL <view>`) requires Slang/Vulkan env vars, a release build, and an external PNG viewer. The audit it serves found 55 findings; the judge loop is minutes when it should be seconds.
...
All game-side types live in `~/Ochroma/projects/urban_horizon` (GAME layer); engine crates gain no game concepts. Fields follow the codebase's existing `pub`-field convention.

```rust
// urban_horizon: src/command/mod.rs (NEW)
...
// urban_horizon: src/walkthrough/mod.rs (NEW)
...
// GAME (urban_horizon) — the surfaces being commanded
...
```rust
// urban_horizon: src/command/mod.rs
impl GameRegistry {
...
// urban_horizon: src/ui/preview.rs
...
// urban_horizon: src/walkthrough/mod.rs
...
|---|---|---|---|
| `GameRegistry::standard()` | `GameView::new` | `urban_horizon/src/bin/play.rs` (~:112) | built once per game session |
...
| Asset Lab panel | new tab in the game's edit surface (M3) | `urban_horizon/src/bin/asset_lab.rs` reusing `forge_pathtrace`'s view/gate fns (read-only — that binary is owned by the porch-closure agent until released; extract shared fns to a module, do not edit it) | gates inline as PASS/FAIL chips |
...
| `Tokens` into the game HUD | color constants in `hud.rs` become `Tokens` reads | `urban_horizon/src/render_gpu/hud.rs` | I9 across repos |
...
Command: `cd ~/Ochroma/projects/urban_horizon && cargo run --release --bin play -- --walkthrough` then `cargo run --release --bin play`
```
W1 — 60 seconds:
...
Command: `cd ~/Ochroma/projects/urban_horizon && LD_LIBRARY_PATH=$HOME/slang-sdk/lib SPECTRA_SLANG_DIR=$HOME/src/spectra/slang SPECTRA_BACKEND=vulkan cargo run --release --features spectra --bin asset_lab`
```
W3 — 60 seconds:
