...
running `cd ~/Ochroma/projects/civitas_care && cargo run --release --bin city_coverage` prints
...
2. **Care coverage / service fields (game, M4).** The identity mechanic of Civitas Care, upgraded from circles to geodesics by §4.6, < 1 ms per rebuild. Highest power-per-effort in the whole design; also the headline demo.
...
- **Game (`civitas_care`)** owns the cook invocation (calls the engine `SdfBaker`), `ReadyAssetSdfVolume` v2 (adds `encoding: Snorm8|Snorm16` + `sign: Closed|Shell`, serde-default so v1 payloads load), placement rules, and all service-field *semantics*. Engine crates never learn the words childcare, lot, or road.
...
|---|---|---|---|
| `SdfBaker::bake` | `ready_sdf` (replaces the CPU O(V·T) body; CPU path stays as the test oracle) | `civitas_care/src/bin/game_asset_cook.rs:1450` | per recipe in the cook loop (line ~65); prints the M1 `[sdf]` + `[sdf-validate]` lines |
...
| `SdfAtlasGpu::insert` | `SceneRenderer::set_city_scene` / asset residency tick | `civitas_care/src/render_gpu/mod.rs` (~2280) | from the same `SpatialFieldRegistry` instances; once per unique asset |
| `SdfClipmap::encode_update` | per-frame before AO, dirty AABBs from develop/plop/destroy | `civitas_care/src/render_gpu/mod.rs` `SceneRenderer::render` | camera recenter + ≤ 5% tile updates (§3 gate) |
| Clearance check | ploppable hover + `place_at` gate | `civitas_care/src/bin/play.rs` (MouseInput chain) + `src/game/mod.rs` | `SpatialFieldRegistry::is_solid(probe, 1.0)` — CPU, no GPU latency |
| Lot/road conform | foundation snap at develop time | `civitas_care/src/render_gpu/mod.rs` `place_ready_asset_*` | terrain-field `project_point` per foundation corner |
| `DistanceTransform2d::encode` | coverage rebuild on facility/road dirty flag | new `civitas_care/src/coverage_field.rs`, ticked from `CivitasGame::tick` | per-service seeds; `read_back` feeds `services` reach numbers |
| Coverage HUD line | `GameView::info_lines` | `civitas_care/src/bin/play.rs:322` | `Childcare reach <R>%` from field readback (M4 gate) |
| SDF AO pass | after expand, before atmosphere composite | `civitas_care/src/render_gpu/mod.rs` + `crates/vox_render/src/gpu/` | half-res march vs clipmap; `sdf_ao` ms printed by scale_trial (M5) |
