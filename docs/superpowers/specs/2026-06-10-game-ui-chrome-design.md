...
- **No time controls.** CS2's bottom-left shows `⏸ PAUSED` + speed steps; Urban Horizon only advances when the player presses Space (`GameView::key`, play.rs — Space → one `game.tick()`). There is no paused/1×/3× model, so the city never lives on its own and the date/population in the status bar only move on keypress.
...
- Normative layout source: `~/Ochroma/projects/urban_horizon/docs/references/ui-layout-spec.md` (user directive, 2026-06-10) — the three-zone bar + submenus-above-the-bar rule this design implements.
