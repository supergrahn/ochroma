...
- **The mask substrate exists but feeds nothing.** `DistanceTransform2d` (SDF Pillar §4.6, shipped in `gpu/distance_field_2d.rs`) produces exactly the per-cell field a density mask needs, and the game already derives `CellClass::{Water, Slope, Buildable}` (`urban_horizon/src/map/layers.rs`) + the clearance-SDF clipmap (SDF Pillar §4.2 Tier-2). No consumer multiplies them into a scatter density. Grass through buildings, roads, and water is the default failure mode of every naive scatter.
...
3. **CellClass gate** — `gate = is_grassable(cellclass(x,z)) ? 1 : 0`, where the game maps `CellClass::{Water, Slope-cliff, Road}` → 0 and `Buildable`/lawn → 1 (`urban_horizon/src/map/layers.rs`). Engine-side this is one `u8` mask lookup, identical machinery to the density field; the game owns the mapping.
...
|---|---|---|---|
| `ScatterField::new_with_context` | `scatter_trial` setup; game `SceneRenderer::new_city` | `crates/vox_app/src/bin/scatter_trial.rs` (new); `urban_horizon/src/render_gpu/mod.rs` | once; shares the building `AssetAtomLibrary` for prototype classes |
| `ScatterField::set_density_field` / `set_cellclass_mask` | game map build / on terrain edit | `urban_horizon/src/render_gpu/mod.rs` | game writes grass density into the 2D field; engine semantic-free |
