...
> **2026-06-06 status:** CPU/crucible GI path is wired live in `EngineLoop::step_gi` (`crates/ochroma_engine/src/engine_loop.rs:334`, test `step_gi_brightens_receiver_band_near_emitter`). The GPU compute upgrade (`GpuGi`, 0e7bdb7) is proven on-device AND wired (9175c71): `GiBackend` on EngineLoop, env/setter-selected, CPU fallback — the rank-3 adoption item is closed.
