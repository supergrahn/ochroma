...
**Build:** `cd ~/Ochroma/projects/urban_horizon && cargo build` / targeted `cargo test --lib <filter>` (lib tests are GPU-free; only `--bin play` runs/tests need the GPU box).

---

## IMPORTANT NOTES

- **Repos & house rules:** ALL code lives in `~/Ochroma/projects/urban_horizon` (GAME layer). The engine repo (`~/src/ochroma`) is never edited, built, or tested by this plan. **Do NOT run `git commit`** — every task's final step is *leave uncommitted*; the user commits.
...
- [x] **Step 5: Run — verify non-trivial output** — run the cook (prints K/M), re-run the lib test (prints dims + distinct colors), and `git -C ~/Ochroma/projects/urban_horizon diff --stat assets/` shows only `pack.json` + new `thumbs/*.png`.
- [x] **Step 6: Leave uncommitted.**
