...
- **NEVER restart the process to see a look change — HOLD THE SCENE OPEN (hard-won 2026-08-01).** Loading a real save costs **~81 400 ms** to reach a **~120 ms** frame — **738:1**, so a restart-per-edit loop spends the day on loading, not looking. `OCHROMA_PRESENT_HOLD=1` on `urban_horizon --present-save-vulkan <save> <out.png> <frames>` keeps the loaded scene alive, polls `dev.ron` + `render.ron`, and re-renders into the same PNG on every save; `touch <out>.stop` ends it. Measured four live edits at **123–147 ms** each vs **81 386 ms** to reload — **620x** — and confirmed in PIXELS (`sun_az` 277.2° → 80.5°), not log lines. Judge the `-internal.png` (native traced res), never an upscaled frame. Covers look/lighting/clock/camera and `.slang` (Slang compiles at runtime); a Rust change still costs a rebuild and an asset/map change still costs a cook + reload. It **exits loudly rather than render stale** when a `render.ron` edit changes baked resident inputs. Costs ~12 GiB resident — run it under `tmux`. **The general lesson, which is the durable part: the hot-reload machinery already existed and simply had no caller on the headless path. Before building a faster loop, check whether the seam is already there and unwired.**
