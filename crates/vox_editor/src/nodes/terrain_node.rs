...
use crate::node_graph::{
    HeightfieldSpatial, NodeDescriptor, NodeError, NodeInputs, NodeOutputs, OchromaNode,
...
fn hydraulic_erode(heights: &mut [f32], resolution: u32, droplet_count: u32, seed: u32) {
    ...
    for _ in 0..droplet_count {
        ...
        for _ in 0..30 {
            ...
            if new_px < 0.0 || new_px >= (n - 1) as f32 || new_py < 0.0 || new_py >= (n - 1) as f32
            {
                break;
            }
...
impl OchromaNode for TerrainNode {
    ...
    fn set_param(&mut self, key: &str, value: ParamValue) -> Result<(), NodeError> {
        match (key, value) {
            ...
            ("world_size", ParamValue::Float(v)) => {
                self.world_size = v as f32;
                Ok(())
            }
            ("amplitude", ParamValue::Float(v)) => {
                self.amplitude = v as f32;
                Ok(())
            }
            ("octaves", ParamValue::Int(v)) => {
                self.octaves = v as usize;
                Ok(())
            }
            ("frequency", ParamValue::Float(v)) => {
                self.frequency = v;
                Ok(())
            }
            ...
            ("droplet_count", ParamValue::Int(v)) => {
                self.droplet_count = v as u32;
                Ok(())
            }
    ...
    fn cook(&self, _inputs: NodeInputs) -> Result<NodeOutputs, NodeError> {
        ...
        if self.resolution < 16 {
            return Err(NodeError::CookFailed(format!(
                "resolution must be >= 16, got {}",
                self.resolution
            )));
        }
        if self.resolution > 256 {
            return Err(NodeError::CookFailed(format!(
                "resolution must be <= 256, got {}",
                self.resolution
            )));
        }
        let mut heights = generate_heightfield(
            self.resolution,
            self.amplitude,
            self.octaves,
            self.frequency,
...
mod tests {
    ...
    fn different_seeds_produce_different_terrain() {
        ...
        assert!(
            diff > 1.0,
            "different seeds should produce different terrain, total diff={}",
            diff
        );
    }
    ...
    fn erosion_modifies_terrain() {
        let no_erosion = TerrainNode {
            resolution: 32,
            droplet_count: 0,
            seed: 42,
            ..Default::default()
        };
        let with_erosion = TerrainNode {
            resolution: 32,
            droplet_count: 1000,
            seed: 42,
            ..Default::default()
        ...
        assert!(
            diff > 0.01,
            "erosion should modify terrain heights, diff={}",
            diff
        );
    }
    ...
    fn negative_resolution_is_rejected_keeping_sane_size() {
        let mut node = TerrainNode {
            resolution: 64,
            droplet_count: 0,
            ..Default::default()
        };
        let err = node
            .set_param("resolution", ParamValue::Int(-5))
            .unwrap_err();
        assert!(matches!(
            err,
            NodeError::UnknownParam(_) | NodeError::CookFailed(_)
        ));
        ...
        assert_eq!(
            hf.heights.len(),
            64 * 64,
            "heightfield size stays the sane 64x64"
        );
    }
