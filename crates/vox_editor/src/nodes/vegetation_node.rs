...
fn emit_cylinder(mesh: &mut EditorMesh, base: [f32; 3], top: [f32; 3], radius: f32) {
    ...
    // Build perpendicular
    let perp = if dir[1].abs() < 0.9 {
        norm3(cross3(dir, [0.0, 1.0, 0.0]))
    } else {
...
impl OchromaNode for VegetationNode {
    ...
    fn set_param(&mut self, key: &str, value: ParamValue) -> Result<(), NodeError> {
        match (key, value) {
            ...
            ("branch_levels", ParamValue::Int(v)) => {
                self.branch_levels = v as u32;
                Ok(())
            }
            ("trunk_radius", ParamValue::Float(v)) => {
                self.trunk_radius = v as f32;
                Ok(())
            }
            ("height", ParamValue::Float(v)) => {
...
mod tests {
    ...
    fn default_node() -> VegetationNode {
    ...
    fn lod0_has_more_triangles_than_lod2() {
        ...
        assert!(
            tri0 > tri2,
            "LOD0 ({} tris) should have more than LOD2 ({} tris)",
            tri0,
            tri2
        );
    }
    ...
    fn lod3_billboard_has_exactly_two_triangles() {
        ...
        assert_eq!(
            lods[3].indices.len(),
            2,
            "LOD3 billboard must be exactly 2 triangles"
        );
    }
    ...
    fn different_seeds_produce_different_trees() {
        let a = VegetationNode {
            seed: 1,
            ..Default::default()
        }
        .cook(NodeInputs::new())
        .unwrap();
        let b = VegetationNode {
            seed: 2,
            ..Default::default()
        }
        .cook(NodeInputs::new())
