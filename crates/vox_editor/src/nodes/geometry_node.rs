...
impl OchromaNode for CatenaryNode {
    ...
    fn set_param(&mut self, key: &str, value: ParamValue) -> Result<(), NodeError> {
        match (key, value) {
            ("slack", ParamValue::Float(v)) => {
                self.slack = v as f32;
                Ok(())
            }
            ("segments", ParamValue::Int(v)) => {
...
impl OchromaNode for PropPlacementNode {
    ...
    fn set_param(&mut self, key: &str, value: ParamValue) -> Result<(), NodeError> {
        match (key, value) {
            ("count", ParamValue::Int(v)) => {
                self.count = v as usize;
                Ok(())
            }
            ("min_clearance", ParamValue::Float(v)) => {
                self.min_clearance = v as f32;
                Ok(())
            }
...
mod tests {
    ...
    fn test_catenary_curve_sags() {
        ...
        assert!(
            mid < 5.0,
            "catenary midpoint should sag below endpoints, got {}",
            mid
        );
    }
