...
fn obs(lat: f32, lon: f32, day: u32, hour: f32) -> Observer {
    Observer { latitude_deg: lat, longitude_deg: lon, day_of_year: day, hour }
}
...
fn the_sun_is_up_by_day_and_the_moon_takes_over_at_night() {
    ...
    assert!(midnight.light().intensity < noon.light().intensity, "moon dimmer than sun");
    assert!(midnight.light().intensity > 0.0, "the moon still gives a little light");
}
...
fn clouds_and_fog_dim_the_light() {
    ...
    assert!(cloudy.light().intensity < clear.light().intensity, "clouds dim the sun");
    assert!(foggy.light().intensity < clear.light().intensity, "fog dims the sun");
...
fn the_sun_crosses_the_sky_east_to_west() {
    ...
    assert!(sunrise.y.abs() < 0.2 && sunset.y.abs() < 0.2, "both near the horizon");
}
...
fn night_geometry_is_dimmer_than_day_geometry() {
    ...
    assert!(lum(night[0]) < lum(day[0]), "the same wall is darker at night");
}

#[test]
fn the_real_sun_is_overhead_at_the_equator_at_an_equinox_noon() {
    let s = solar_dir(obs(0.0, 0.0, 80, 12.0));
    assert!(s.y > 0.98, "equator equinox noon: sun overhead, alt sin={}", s.y);
    let mid = solar_dir(obs(0.0, 0.0, 80, 0.0));
    assert!(mid.y < -0.9, "equator midnight: sun below horizon, {}", mid.y);
}
...
fn summer_noon_sun_is_higher_than_winter_noon_in_the_north() {
    ...
    assert!(summer > winter, "summer noon higher than winter: {summer} vs {winter}");
    assert!(summer > 0.7, "high summer sun in Oslo");
    assert!(winter > 0.0 && winter < 0.25, "low winter sun in Oslo, just above the horizon");
}
...
fn a_real_polar_summer_keeps_the_sun_up_at_midnight() {
    ...
    assert!(s.y > 0.0, "midnight sun above the Arctic Circle, alt sin={}", s.y);
}
