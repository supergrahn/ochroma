...
use vox_data::{ImportSettings, SpectralMaterialDb, import_asset};
...
fn ochroma_tools_v3_path_writes_loadable_file() {
    ...
    let loaded =
        VxmFile::read(&bytes[..]).expect("VxmFile::read must load ochroma-tools v3 output");
    ...
    for (i, (got, want)) in loaded.splats.iter().zip(&want_positions).enumerate() {
        assert_eq!(
            got.position(),
            *want,
            "splat {i} position mismatch after reload"
        );
    }
