...
fn render_terrain_from_above_produces_visible_image() {
    ...
    println!(
        "Terrain from above: {} splats, {:.1}% coverage, saved to {}",
        terrain.len(),
        coverage,
        dir.join("terrain_above.ppm").display()
    );
    assert!(
        coverage > 10.0,
        "Terrain should cover >10% of image: {:.1}%",
        coverage
    );
}
...
fn render_buildings_at_street_level() {
    ...
    println!(
        "Street level: {} splats, {:.1}% coverage",
        splats.len(),
        coverage
    );
    assert!(
        coverage > 5.0,
        "Buildings should be visible: {:.1}%",
        coverage
    );
}
...
fn render_trees_have_green_content() {
    ...
    println!(
        "Tree colour: R={:.0} G={:.0} B={:.0} ({} coloured pixels)",
        avg_r,
        avg_g,
        avg_b,
        coloured.len()
...
fn different_illuminants_change_appearance() {
    ...
    let differs = fb_d65
        .pixels
        .iter()
        .zip(fb_a.pixels.iter())
        .any(|(a, b)| a[0] != b[0] || a[1] != b[1] || a[2] != b[2]);
    assert!(
        differs,
        "D65 and Illuminant A should produce different colours"
    );
}
...
fn render_detailed_building_windows_differ_from_walls() {
    ...
    let building =
        vox_data::proc_gs_advanced::generate_detailed_building(42, 8.0, 10.0, 3, "victorian");
    assert!(
        building.len() > 500,
        "Detailed building should have many splats: {}",
        building.len()
    );
...
fn render_detailed_building_modern_style() {
    let building =
        vox_data::proc_gs_advanced::generate_detailed_building(99, 10.0, 8.0, 5, "modern");
    ...
    println!(
        "Modern building: {} splats, {:.1}% coverage",
        building.len(),
        coverage
    );
...
fn render_with_spectra_renderer() {
    ...
    assert!(
        coverage > 1.0,
        "Spectra renderer should produce some visible pixels: {:.1}%",
        coverage
    );
}
