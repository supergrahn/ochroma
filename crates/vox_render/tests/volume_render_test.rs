...
fn render_volumetric_terrain_with_cave_and_cliff() {
    ...
    println!(
        "Volumetric terrain: {} surface splats from {} solid voxels",
        splats.len(),
        vol.solid_count()
    );
    ...
    println!(
        "Volumetric terrain render: {:.1}% coverage, saved to {}",
        coverage,
        dir.join("volumetric_terrain.ppm").display()
    );

    assert!(splats.len() > 500, "Should have surface splats");
    assert!(
        coverage > 5.0,
        "Terrain should be visible: {:.1}%",
        coverage
    );
}
