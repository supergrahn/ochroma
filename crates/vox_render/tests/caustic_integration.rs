...
use glam::Vec3;
use vox_render::spectral_caustics::{BAND_UM, CauchyGlass, SpectralCaustics};
...
fn prism_separates_white_light_violet_to_red() {
    ...
    for b in 0..15 {
        assert!(
            ...
            b,
            (BAND_UM[b] * 1000.0) as u32,
            x_components[b],
            b + 1,
            (BAND_UM[b + 1] * 1000.0) as u32,
            x_components[b + 1]
        );
    }
}
...
fn chromatic_spread_increases_with_incidence_angle() {
    ...
    println!(
        "spread at 10°: {:.6} rad, at 45°: {:.6} rad",
        spread_10, spread_45
    );
    assert!(
        spread_45 > spread_10,
...
fn n_bk7_dispersion_matches_published_values() {
    ...
    assert!(
        ...
        "N-BK7 dispersion should be ~0.017, got {:.4}",
        dispersion
    );
}
