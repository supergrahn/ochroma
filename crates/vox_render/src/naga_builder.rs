...
use naga::{
    Arena, Expression, Function, FunctionResult, Handle, Literal, Module, ScalarKind, Span,
    Statement, Type, TypeInner, UniqueArena,
};
...
impl NagaBuilder {
        ...
        types.insert(
            Type {
                ...
                inner: TypeInner::Array {
                    base: f32_ty,
                    size: naga::ArraySize::Constant(std::num::NonZeroU32::new(16).unwrap()),
                    stride: 4,
                },
            },
            Span::UNDEFINED,
    ...
    pub fn emit_constant_spd(
        ...
        let components: Vec<Handle<Expression>> = spd
            .iter()
            .map(|&v| expressions.append(Expression::Literal(Literal::F32(v)), Span::UNDEFINED))
            .collect();
    ...
    pub fn build_constant(spd: [f32; 16]) -> Module {
        ...
        let (expr, compose_start) =
            Self::emit_constant_spd(&spd, &mut func.expressions, &mut module.types);
...
mod tests {
    ...
    fn constant_module_validates() {
        let spd = [
            0.8, 0.7, 0.6, 0.5, 0.5, 0.6, 0.7, 0.8, 0.8, 0.7, 0.6, 0.5, 0.5, 0.6, 0.7, 0.8,
        ];
    ...
    fn module_has_evaluate_material_function() {
        ...
        assert!(
            ...
            "expected 'evaluate_material' function, got: {:?}",
            names
        );
    }
