...
use super::super::*;
    ...
    fn denoise_lowspp_demo() {
        ...
        let (w, h) = (1280u32, 720u32);
        let tag = if std::env::var("OCHROMA_DENOISE_OFF").is_ok() { "raw" } else { "denoised" };
        for spp in [1u32, 4u32] {
            let t0 = std::time::Instant::now();
            let rgba = pathtrace_mesh_lit_weathered_to_rgba(
                &mesh.positions, &mesh.normals, &mesh.uvs, &mesh.indices, &mesh.material_ids,
                &mats, &texs, eye, target, 0.85, w, h, spp, &rig, &[],
            )
            ...
            write_png_rgba(out.to_str().unwrap(), &flat, w, h);
            eprintln!("[lowspp] {spp} spp ({tag}) -> {} in {dt:.2}s", out.display());
        }
    }
    ...
    fn spectra_resident_realtime_bench() {
        ...
        for (label, w, h, bounces) in configs {
            match spectra_resident_bench(
                &mesh.positions, &mesh.normals, &mesh.uvs, &mesh.indices, &mesh.material_ids,
                &mats, &texs, eye, target, 0.85, w, h, 1, bounces, frames, &rig,
            ) {
                Ok(r) => {
                    ...
                    eprintln!(
                        ...
                        r.cold_ms, p50, fps, r.steady_ms.len()
                    );
                    if w == 427 {
    ...
    fn render_curved_hospital_nodedag() {
        use super::{pathtrace_mesh_lit_to_rgba, LightRig, LookPreset, PbrMaterial};
        ...
        for line in text.lines() {
            ...
            match it.next() {
                Some("v") => {
                    ...
                    if v.len() == 3 { positions.push([v[0], v[1], v[2]]); }
                }
                Some("vn") => {
                    let v: Vec<f32> = it.take(3).filter_map(|s| s.parse().ok()).collect();
                    if v.len() == 3 { normals.push([v[0], v[1], v[2]]); }
                }
                Some("f") => {
                    ...
                    let idx: Vec<u32> = it
                        ...
                        .map(|i| i - 1)
                        .collect();
                    if idx.len() == 3 { indices.push([idx[0], idx[1], idx[2]]); }
                }
                _ => {}
            }
        }
        assert!(positions.len() > 1000 && indices.len() > 1000, "OBJ too small: {} verts {} tris", positions.len(), indices.len());
        if normals.len() != positions.len() {
            normals = vec![[0.0, 1.0, 0.0]; positions.len()];
        }
        let uvs = vec![[0.0f32, 0.0]; positions.len()];
        let material_ids = vec![0u8; positions.len()];
        let mats = vec![PbrMaterial { base_color: [0.74, 0.74, 0.78], roughness: 0.6, ..Default::default() }];

        // Frame the whole building (mesh spans ~x[0,72], y[0,55], z varies).
        let (mut lo, mut hi) = ([f32::INFINITY; 3], [f32::NEG_INFINITY; 3]);
        for p in &positions { for a in 0..3 { lo[a] = lo[a].min(p[a]); hi[a] = hi[a].max(p[a]); } }
        let c = [(lo[0]+hi[0])*0.5, (lo[1]+hi[1])*0.5, (lo[2]+hi[2])*0.5];
        ...
        let target = c;
        let rig = LightRig {
            sun_dir: [0.4, 0.6, 0.5], sun_color: [1.0, 0.95, 0.85], sun_intensity: 5.0, sun_radiance: 30.0,
            sky_intensity: 0.3, camera_fill: 0.2, rim_fill: 0.15, atmosphere_enabled: true, atmosphere_turbidity: 2.5,
            sky_dome_intensity: 0.5, sky_dome_zenith: [0.42,0.55,0.78], sky_dome_horizon: [0.82,0.85,0.90],
            look: LookPreset::SoftReview, ..Default::default()
        };
        let (w, h) = (1280u32, 720u32);
        let t0 = std::time::Instant::now();
        let rgba = pathtrace_mesh_lit_to_rgba(
            &positions, &normals, &uvs, &indices, &material_ids, &mats, &[], eye, target, 0.8, w, h, 8, &rig,
        ).expect("render curved hospital");
        let flat: Vec<u8> = rgba.chunks_exact(4).flat_map(|p| [p[0], p[1], p[2], 255]).collect();
        let out = std::env::temp_dir().join("curved_hospital_render.png");
        write_png_rgba(out.to_str().unwrap(), &flat, w, h);
        eprintln!("[curved] {} verts {} tris -> {} in {:.1}s", positions.len(), indices.len(), out.display(), t0.elapsed().as_secs_f32());
    }
    ...
    fn pom_demo() {
        ...
        let render = |mats: &[super::PbrMaterial]| -> Vec<u8> {
            let rgba = pathtrace_mesh_lit_weathered_to_rgba(
                &mesh.positions, &mesh.normals, &mesh.uvs, &mesh.indices, &mesh.material_ids,
                mats, &texs, eye, target, fov_y, w, h, spp, &rig, &[],
            )
            .expect("pom render");
            let pxv: Vec<[u8; 4]> =
                rgba.chunks_exact(4).map(|p| [p[0], p[1], p[2], 255]).collect();
        ...
        let render_t = |mats: &[super::PbrMaterial]| -> f64 {
            ...
            let _ = pathtrace_mesh_lit_weathered_to_rgba(
                &mesh.positions, &mesh.normals, &mesh.uvs, &mesh.indices, &mesh.material_ids,
                mats, &texs, eye, target, fov_y, w, h, cost_spp, &rig, &[],
            )
        ...
        write_png_rgba(std::env::temp_dir().join("pom_off.png").to_str().unwrap(), &off, w, h);
        write_png_rgba(std::env::temp_dir().join("pom_on.png").to_str().unwrap(), &on, w, h);
    ...
    fn cone_step_demo() {
        ...
        for &tid in &height_tex_ids {
            ...
            let var = cones.iter().map(|&c| (c as f64 - mean).powi(2)).sum::<f64>() / n;
            let (cmin, cmax) = cones
                .iter()
        ...
        write_png_rgba(std::env::temp_dir().join("cone_off.png").to_str().unwrap(), &off, w, h);
        write_png_rgba(std::env::temp_dir().join("cone_on.png").to_str().unwrap(), &on, w, h);
    ...
    fn forge_nodedag_textured_lot() {
        use super::{pathtrace_mesh_lit_to_rgba, LightRig, LookPreset, PbrMaterial, TextureImage};
        ...
        for (label, idx) in [
            ...
            eprintln!("[nodedag_lot] {label} diffuse loaded {}x{} (1K)", t.width, t.height);
        }
    ...
    fn forge_nodedag_textured_closeup() {
        ...
        let mut uvs: Vec<[f32; 2]> = uv_text
            ...
            .collect();
        let mut material_ids: Vec<u8> =
            mat_text.lines().filter_map(|l| l.trim().parse::<u8>().ok()).collect();
        if normals.len() != positions.len() {
            normals = vec![[0.0, 1.0, 0.0]; positions.len()];
        }
        ...
        let ph_root =
            dirs_next_cache().map(|c| c.join("aetherspectra/polyhaven")).expect("cache dir");
        let mut textures: Vec<TextureImage> = Vec::new();
        let mut load = |set: &str, kind: TexKind, tint: [f32; 3]| -> i32 {
        ...
        const LOT_ID: u8 = 8;
        let materials =
            vec![facade, roof, glass, trim, trim, trim, door, glass_lit, lot_mat];
        ...
        push_quad(
            [[-3.0, 0.3, -3.0], [-3.0, 0.3, 43.0], [75.0, 0.3, 43.0], [75.0, 0.3, -3.0]],
            [0.0, 1.0, 0.0],
            3,
            2.5,
        );
        ...
        let target = [c[0] - span * 0.05, lo[1] + (hi[1] - lo[1]) * 0.28, c[2] - span * 0.02];
        let rig = LightRig {
        ...
        let crop = |x0: u32, y0: u32, x1: u32, y1: u32| -> (f64, f64, [f64; 3]) {
            ...
            for y in y0..y1 {
                for x in x0..x1 {
                    let i = ((y * w + x) * 4) as usize;
                    let (r, g, b) =
                        (opaque[i] as f64, opaque[i + 1] as f64, opaque[i + 2] as f64);
                    let lum = 0.2126 * r + 0.7152 * g + 0.0722 * b;
                    sum += lum;
                    sum2 += lum * lum;
                    rgb[0] += r;
            ...
            let nf = n as f64;
            let mean = sum / nf;
            (mean, (sum2 / nf - mean * mean).max(0.0).sqrt(), [rgb[0] / nf, rgb[1] / nf, rgb[2] / nf])
        };
    ...
    fn cone_step_relief_verify() {
        use super::{
            load_polyhaven_map, pathtrace_mesh_lit_weathered_to_rgba, write_png_rgba, LightRig,
            PbrMaterial, TexKind,
        };
        ...
        let cvar = cones.iter().map(|&c| (c as f64 - cmean).powi(2)).sum::<f64>() / cn;
        let (cmin, cmax) = cones
            .iter()
        ...
        let render = |mats: &[PbrMaterial], eye: [f32; 3], tgt: [f32; 3]| -> Vec<u8> {
            pathtrace_mesh_lit_weathered_to_rgba(
                &positions, &normals, &uvs, &indices, &material_ids, mats, &textures, eye, tgt,
                fov_y, w, h, spp, &rig, &[],
            )
        ...
        write_png_rgba(
            std::env::temp_dir().join("cone_step_relief.png").to_str().unwrap(),
            &cone, w, h,
        );
        write_png_rgba(
            std::env::temp_dir().join("cone_step_relief_flat.png").to_str().unwrap(),
            &flat, w, h,
        );
        write_png_rgba(
            std::env::temp_dir().join("cone_step_relief_grazing.png").to_str().unwrap(),
            &grazing, w, h,
        );
        ...
        let render_t = |mode: &str| -> f64 {
            ...
            let t = std::time::Instant::now();
            let _ = pathtrace_mesh_lit_weathered_to_rgba(
                &positions, &normals, &uvs, &indices, &material_ids, &[mat_on], &textures,
                eye_headon, target, fov_y, w, h, cost_spp, &rig, &[],
            )
    ...
    fn hospital_full_capability() {
        use super::{pathtrace_mesh_lit_weathered_to_rgba, LightRig, PbrMaterial, TextureImage};
        ...
        eprintln!(
            ...
            materials[0].displacement_tex, materials[0].displacement_scale, materials[0].displacement_midlevel
        );
        ...
        let half = 100.0f32;
        push_quad(
            &mut positions, &mut normals, &mut uvs, &mut indices, &mut material_ids, &mut masks,
            [
                [cx - half, 0.0, cz - half],
                [cx - half, 0.0, cz + half],
                [cx + half, 0.0, cz + half],
        ...
        let render = |masks: &[f32]| -> Vec<u8> {
            let rgba = pathtrace_mesh_lit_weathered_to_rgba(
                &positions, &normals, &uvs, &indices, &material_ids,
                &materials, &textures, eye, target, fov_y, rw, rh, spp, &rig, masks,
            )
        ...
        eprintln!(
            ...
            frgb[0], frgb[1], frgb[2], frgb[0] - frgb[2]
        );
        assert!(
            frgb[0] > frgb[2] + 4.0,
        ...
        assert!(
            ...
            off_mean[0], off_mean[1], off_mean[2]
        );
        ...
        eprintln!(
            ...
            frgb[0] - frgb[2], weather_delta, spectral_delta, spectral_delta_full,
            materials[0].displacement_tex, materials[0].displacement_scale, max_x
        );
    }
    ...
    fn hospital_realtime_fsr() {
        use super::super::{
            spectra_resident_bench_fsr, LightRig, PbrMaterial, TextureImage,
        };
        ...
        let uv = [1.0f32, 1.0];
        let facade = PbrMaterial {
            base_color: [0.62, 0.30, 0.24], roughness: 0.9,
            albedo_tex: fac_d, normal_tex: fac_n, roughness_tex: fac_r,
            displacement_tex: fac_h, displacement_scale: 0.04, displacement_midlevel: 0.5,
            uv_scale: uv, ..Default::default()
        };
        let roof = PbrMaterial {
            base_color: [0.30, 0.31, 0.34], roughness: 0.8,
            albedo_tex: roof_d, normal_tex: roof_n, roughness_tex: roof_r,
            uv_scale: uv, ..Default::default()
        };
        let glass = PbrMaterial {
            base_color: [0.78, 0.86, 0.92], roughness: 0.04,
            transmission: 1.0, ior: 1.5, thin_walled: true, ..Default::default()
        };
        let trim = PbrMaterial {
            base_color: [0.60, 0.60, 0.58], roughness: 0.75,
            albedo_tex: trim_d, normal_tex: trim_n, roughness_tex: trim_r,
            displacement_tex: trim_h, displacement_scale: 0.04, displacement_midlevel: 0.5,
            uv_scale: uv, ..Default::default()
        };
        let door = PbrMaterial {
            base_color: [0.50, 0.50, 0.52], roughness: 0.4, metallic: 0.9,
            albedo_tex: door_d, normal_tex: door_n, roughness_tex: door_r,
            uv_scale: uv, ..Default::default()
        };
        let glass_lit = PbrMaterial {
            base_color: [1.0, 0.82, 0.58], emission_strength: 1.6, ..Default::default()
        };
        let lot_mat = PbrMaterial {
            base_color: [0.55, 0.52, 0.46], roughness: 0.95,
            albedo_tex: lot_d, normal_tex: lot_n, roughness_tex: lot_r,
            uv_scale: uv, ..Default::default()
        };
        const LOT_ID: u8 = 8;
        let materials = vec![facade, roof, glass, trim, trim, trim, door, glass_lit, lot_mat];
        ...
        push_quad(
            ...
            [[cx - half, 0.0, cz - half], [cx - half, 0.0, cz + half],
             [cx + half, 0.0, cz + half], [cx + half, 0.0, cz - half]],
            [0.0, 1.0, 0.0], LOT_ID, 2.5,
        );
        push_quad(
            &mut positions, &mut normals, &mut uvs, &mut indices, &mut material_ids, &mut masks,
            [[-3.0, 0.3, -3.0], [-3.0, 0.3, 43.0], [63.0, 0.3, 43.0], [63.0, 0.3, -3.0]],
            [0.0, 1.0, 0.0], 3, 2.5,
        );
        ...
        eprintln!(
            ...
            positions.len(), tris as f32 / 1000.0
        );
        ...
        let run = |q: UpscaleQuality, spp: u32| {
            spectra_resident_bench_fsr(
                &positions, &normals, &uvs, &indices, &material_ids,
                &materials, &textures, eye, target, fov_y,
                1280, 720, q, spp, 3, 8, &rig,
            )
        ...
        for (label, q) in [("640x360", UpscaleQuality::Performance),
            ...
            for spp in [2u32, 4u32] {
                ...
                eprintln!(
                    ...
                    label, spp, rmed, fmed, emed, 1000.0 / emed, rep.cold_ms,
                    rep.render_w, rep.render_h
                );
                // Keep the 640x360 @ 4spp frame as the headline PNG.
                if label == "640x360" && spp == 4 {
                    best_png = Some((rep.upscaled.clone(), rep.out_w, rep.out_h,
                        format!("{}x{} internal, 4 spp, denoise+FSR->{}x{}",
        ...
        if let Some((px, w, h, desc)) = &best_png {
            ...
            assert!(mean > 4.0 && frac > 0.2,
                "FSR upscaled frame degenerate (mean {mean:.1}, lit {:.0}%)", frac * 100.0);
        }
        ...
        unsafe { std::env::remove_var("OCHROMA_LEAN"); }
        let full = run(q, 4);
        let full_e2e = median(&full.e2e_ms);
        let full_render = median(&full.render_ms);
        // Lean (ReSTIR off).
        unsafe { std::env::set_var("OCHROMA_LEAN", "1"); }
        let lean = run(q, 4);
        ...
        eprintln!(
            "[rt] speedup(e2e) {speedup:.3}x | mean |Δrgb| (full vs lean) = {delta:.3}/255"
        );
        ...
        assert!(
            full.upscaled.iter().step_by(4).map(|&x| x as u64).sum::<u64>() > 0
                && lean.upscaled.iter().step_by(4).map(|&x| x as u64).sum::<u64>() > 0,
    ...
    fn hospital_fsr_1080p_1440p_deliverable() {
        ...
        let run = |out_w: u32, out_h: u32, q: UpscaleQuality, spp: u32| {
            spectra_resident_bench_fsr(
                &scene.positions, &scene.normals, &scene.uvs, &scene.indices,
                &scene.material_ids, &scene.materials, &scene.textures,
                scene.eye, scene.target, scene.fov_y,
                out_w, out_h, q, spp, 3, 8, &scene.rig,
            )
            .expect("FSR resident bench")
        };
        ...
        eprintln!("[del] reference = 32spp, à-trous denoiser ON (converged truth, same normalized path)");

        for spp in [2u32, 4u32] {
            // BEFORE: legacy single 5×5 (1 pass, no luminance edge-stop).
            unsafe { std::env::set_var("OCHROMA_DENOISE_LEGACY", "1"); }
            let before = run(1280, 720, UpscaleQuality::Performance, spp);
            unsafe { std::env::remove_var("OCHROMA_DENOISE_LEGACY"); }
            ...
            // for visual A/B.
            if spp == 4 {
                write_png_rgba("/tmp/hospital_fixed_B.png", &after.upscaled, after.out_w, after.out_h);
                write_png_rgba("/tmp/hospital_legacy_5x5.png", &before.upscaled, before.out_w, before.out_h);
                write_png_rgba("/tmp/hospital_ref_32spp.png", &reference.upscaled, reference.out_w, reference.out_h);
                eprintln!("[del] wrote /tmp/hospital_fixed_B.png (à-trous 4spp), /tmp/hospital_legacy_5x5.png, /tmp/hospital_ref_32spp.png");
            }
        }
                ...
                for spp in [2u32, 4u32] {
                    ...
                    let better = |cur: &Option<(String, u32, (u32, u32), f64)>| -> bool {
                        match cur {
                            None => true,
                            Some((_, s, (cw, ch), _)) => {
                                cand_rank > (cw * ch) as u64 * 10 + *s as u64
                            }
                        }
                    };
                    if fps >= 30.0 {
                        if oname == "1080p" && better(&best_1080) {
                            best_1080 = Some((qname.to_string(), spp, (rep.render_w, rep.render_h), fps));
                        }
                        if oname == "1440p" && better(&best_1440) {
                            best_1440 = Some((qname.to_string(), spp, (rep.render_w, rep.render_h), fps));
                        }
                    }
                }
            }
        ...
        for (oname, ow, oh) in outs {
            for spp in [2u32, 4u32] {
                ...
                eprintln!(
                    ...
                    median(&rep.render_ms), median(&rep.fsr_ms), rep.render_w, rep.render_h
                );
                if fps >= 30.0 {
                    let cand = Some((format!("UltraPerf+LEAN"), spp, (rep.render_w, rep.render_h), fps));
                    if oname == "1080p" && best_1080.is_none() { best_1080 = cand.clone(); }
                    if oname == "1440p" && best_1440.is_none() { best_1440 = cand; }
                }
            }
        }
        unsafe {
    ...
    fn hospital_fsr_moving_camera_ab() {
        ...
        let run = || {
            spectra_resident_bench_fsr(
                ...
                &scene.material_ids, &scene.materials, &scene.textures,
                scene.eye, scene.target, scene.fov_y,
                1280, 720, UpscaleQuality::Performance, 4, 3, 8, &scene.rig,
            )
        ...
        let buggy = run();
        unsafe { std::env::remove_var("OCHROMA_FSR_LEGACY_JITTER"); }
        // FIXED: one jitter source (0,0) + real bridged pixel-space MV.
        let fixed = run();
        unsafe { std::env::remove_var("OCHROMA_BENCH_CAM_MOTION"); }

        write_png_rgba("/tmp/hospital_moving_buggy.png", &buggy.upscaled, buggy.out_w, buggy.out_h);
        write_png_rgba("/tmp/hospital_moving_fixed.png", &fixed.upscaled, fixed.out_w, fixed.out_h);
        ...
        assert!(
            buggy.upscaled.iter().step_by(4).map(|&x| x as u64).sum::<u64>() > 0
                && fixed.upscaled.iter().step_by(4).map(|&x| x as u64).sum::<u64>() > 0,
    ...
    fn load_hospital_v2_scene() -> HospitalScene {
        ...
        for l in w_text.lines() {
            for tok in l.split_whitespace() {
                if let Ok(x) = tok.parse::<f32>() { masks.push(x); }
            }
        }
        let building_verts = positions.len();
        if normals.len() != positions.len() {
        ...
        HospitalScene {
            positions, normals, uvs, indices, material_ids, materials, textures,
            eye, target, fov_y: 0.7, rig: LightRig::realistic_daylight(),
        }
    }
    ...
    fn hospital_tlas_build_ms() {
        ...
        let obj_text = std::fs::read_to_string("/tmp/hospital_v2.obj")
            .expect("/tmp/hospital_v2.obj not found");
        let mut positions: Vec<[f32; 3]> = Vec::new();
        let mut normals: Vec<[f32; 3]> = Vec::new();
        let mut indices: Vec<[u32; 3]> = Vec::new();
        for line in obj_text.lines() {
        ...
        for r in 1..reps {
            ...
            for p in &positions { big_pos.push([p[0] + dx, p[1], p[2]]); }
            big_nrm.extend_from_slice(&normals);
            big_uv.extend_from_slice(&uvs);
            for t in &indices { big_idx.push([t[0] + off, t[1] + off, t[2] + off]); }
        }
        ...
        if built_small && built_big {
            ...
            eprintln!(
                ...
                tris_big / tris_small.max(1), scale
            );
        } else {
    ...
    fn hospital_lean_parity() {
        use super::super::{pathtrace_mesh_lit_weathered_to_rgba, LightRig, PbrMaterial};
        ...
        let material_ids: Vec<u8> = (0..indices.len())
            ...
            .collect();
        let materials = vec![
            PbrMaterial { base_color: [0.62, 0.30, 0.24], roughness: 0.9, ..Default::default() },
            PbrMaterial { base_color: [0.30, 0.31, 0.34], roughness: 0.8, ..Default::default() },
            PbrMaterial {
                base_color: [0.78, 0.86, 0.92], roughness: 0.04,
                transmission: 1.0, ior: 1.5, thin_walled: true, ..Default::default()
            },
        ...
        let render = |lean: bool| -> Vec<u8> {
            ...
            pathtrace_mesh_lit_weathered_to_rgba(
                &positions, &normals, &uvs, &indices, &material_ids,
                &materials, &[], eye, target, 0.7, w, h, spp, &rig, &masks,
            )
