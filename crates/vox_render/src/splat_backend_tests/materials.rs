...
fn forge_facade_materials() {
    ...
    let luma = |c: [f64; 3]| -> f64 { (0.2126 * c[0] + 0.7152 * c[1] + 0.0722 * c[2]) / 255.0 };
    ...
    assert!(
        worst_secs < 10.0,
        "gate renders are not cheap: {worst_secs:.2}s"
    );
}
...
fn forge_facade_zoning_closeup() {
    ...
    assert!(
        nv > 0,
        "no side-wall glass verts found (id 2 near x={xmax})"
    );
    let target = [
    ...
    let opaque = |v: Vec<u8>| -> Vec<u8> {
        let px: Vec<[u8; 4]> = v.chunks_exact(4).map(|c| [c[0], c[1], c[2], 255]).collect();
