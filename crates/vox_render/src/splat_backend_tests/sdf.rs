...
fn sdf_building_renders_solid_surface() {
    ...
    let splat_coverage = if std::env::var("OCHROMA_SKIP_SPLAT_COMPARE").is_ok() {
        ...
        let dist_at =
            |x: u32, y: u32, z: u32| -> f32 { volume.distances[(x + nx * (y + ny * z)) as usize] };
        ...
        match super::pathtrace_splats_to_rgba(&splats, eye, center, fov_y, w, h, 16, rig.sun_dir) {
            Ok(srgba) => {
...
fn sdf_craftsman_atom_material_and_glass() {
    use super::{LightRig, SdfSceneInstance, pathtrace_sdf_scene_with_atoms_to_rgba};
    ...
    assert!(
        raw_glass > 50,
        "expected many cooked glass atoms, got {raw_glass}"
    );
...
fn sdf_gather_grid_matches_linear() {
    use super::{pathtrace_sdf_scene_textured_to_rgba, pathtrace_sdf_scene_with_atoms_to_rgba};
    ...
    eprintln!(
        "[sdf_gather_grid] wrote {} (linear oracle)",
        linear_png.display()
    );
...
fn sdf_channel_material_packing() {
    use super::{
        PbrMaterial, SDF_ATOM_CH_FACADE, SDF_ATOM_CH_GLASS, SDF_ATOM_CH_ROOF, SdfChannelMaterials,
        SdfUvParams, pack_sdf_channel_material_table, pack_sdf_uv_params,
    };
    ...
    for inst in packed.chunks_exact(9) {
        assert_eq!(
            inst[SDF_ATOM_CH_FACADE as usize], 0.0,
            "facade slot -> material 0"
        );
        assert_eq!(
            inst[SDF_ATOM_CH_ROOF as usize], 1.0,
            "roof slot -> material 1"
        );
        assert_eq!(
            inst[SDF_ATOM_CH_GLASS as usize], -1.0,
            "glass slot stays -1 (M2 BSDF)"
        );
    }
...
fn sdf_craftsman_textured_quality() {
    ...
    eprintln!(
        "[sdf_textured] wrote {} (M2 flat control)",
        flat_png.display()
    );
...
fn sdf_city_block_renders_multiple_solid_buildings() {
    ...
    let t0 = std::time::Instant::now();
    let rgba = pathtrace_sdf_scene_to_rgba(&volumes, &instances, eye, center, fov_y, w, h, 4, &rig)
        ...
        for cx in [lmin[0], lmax[0]] {
            for cy in [lmin[1], lmax[1]] {
                for cz in [lmin[2], lmax[2]] {
                    let world = pos + q * (glam::Vec3::new(cx, cy, cz) * inst.uniform_scale);
