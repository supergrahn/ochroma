...
use crate::gpu::skinning_compute::{GpuJointTransform, GpuSkinSplat};
...
impl BlendSkinningCompute {
    pub fn new(
        ...
        let identity = GpuJointTransform {
            skin_matrix: glam::Mat4::IDENTITY.to_cols_array_2d(),
        };
        let identity_data: Vec<GpuJointTransform> = vec![identity; jc];

        let default_splat = [GpuSkinSplat::zeroed()];
        let base_splat_contents: &[GpuSkinSplat] = if base_splats.is_empty() {
            &default_splat
        } else {
            base_splats
        };
        ...
        let joint_binding_contents: &[u32] = if joint_bindings.is_empty() {
            &default_binding
        } else {
            joint_bindings
        };
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: pose_buffers[0].as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: pose_buffers[1].as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: pose_buffers[2].as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
                    resource: pose_buffers[3].as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 6,
