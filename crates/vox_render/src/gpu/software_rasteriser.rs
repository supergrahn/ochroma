...
use spectra_gaussian_render::renderer::{
    ALPHA_THRESHOLD, Gaussian3D, ProjectedGaussian, TRANSMITTANCE_THRESHOLD,
    project_gaussian,
};
...
// `build_gaussian_camera` was lifted out of this (banned) module into the
// ungated `crate::gpu::gaussian_camera` module — it is pure CPU camera math, and
// the Spectra product compositors need it without compiling this rasterizer.
// Re-exported here so the rasterizer's own call sites are unchanged.
pub(crate) use crate::gpu::gaussian_camera::build_gaussian_camera;
