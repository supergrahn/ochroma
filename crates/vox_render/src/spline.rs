...
impl RoadSpline {
    pub fn new(control_points: Vec<Vec3>, width: f32) -> Self {
        Self {
            spline: CatmullRomSpline::new(control_points),
            width,
            material_name: "road_asphalt".into(),
        }
    }
    ...
    pub fn sample_edges(&self, count: usize) -> (Vec<Vec3>, Vec<Vec3>) {
        ...
        let left: Vec<Vec3> = center_points
            .iter()
            .enumerate()
            .map(|(i, &c)| {
            ...
            .collect();
        let right: Vec<Vec3> = center_points
            .iter()
            .enumerate()
            .map(|(i, &c)| {
                let t = i as f32 / count.max(1) as f32 * total_t;
...
mod tests {
    ...
    fn catmull_rom_sample_midpoint_is_between_endpoints() {
        ...
        assert!(
            ...
            "midpoint.x={} should be between {} and {}",
            mid.x,
            start.x,
            end.x
        );
