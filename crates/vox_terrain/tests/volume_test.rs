...
fn add_sphere_creates_solid() {
    ...
    assert!(
        vol.solid_count() > 100,
        "Sphere should create solid voxels: {}",
        vol.solid_count()
    );
}
...
fn volume_to_splats_produces_surface_only() {
    ...
    assert!(
        ...
        "Splats should only be on surface, not interior"
    );
}
...
fn demo_volume_has_all_features() {
    ...
    assert!(
        vol.solid_count() > 1000,
        "Demo should have significant solid"
    );
