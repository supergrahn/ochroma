...
pub fn generate_terrain_splats(terrain: &TerrainPlane, material: &str) -> Vec<GaussianSplat> {
    ...
    let spd: [f32; 16] = match material {
        "grass" => [
            0.03, 0.03, 0.04, 0.04, 0.05, 0.06, 0.10, 0.38, 0.36, 0.22, 0.14, 0.08, 0.06, 0.42,
            0.46, 0.48,
        ],
        "cobblestone" => [
            0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.17, 0.18, 0.18, 0.18, 0.17, 0.17, 0.16, 0.16,
            0.15, 0.15,
        ],
        _ => [
            0.03, 0.03, 0.04, 0.04, 0.04, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.06, 0.06,
            0.06, 0.06,
        ],
    };
    ...
    for ix in 0..nx {
        for iz in 0..nz {
            let pos = [
                ix as f32 * spacing - terrain.width * 0.5,
                0.0,
                iz as f32 * spacing - terrain.depth * 0.5,
            ];
