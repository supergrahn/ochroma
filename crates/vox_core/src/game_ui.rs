...
impl GameUI {
    ...
    pub fn render_to_pixels(&self, pixels: &mut [[u8; 4]], width: u32, height: u32) {
        ...
        for el in &self.elements {
            ...
            let (x, y) = match &el.position {
                ...
                UIPosition::Center => (width.saturating_sub(tw) / 2, height.saturating_sub(th) / 2),
    ...
    fn render_centered_lines(
        ...
        let line_gap: u32 = 12;
        let total_h: u32 = lines.iter().map(|(_, sz)| CHAR_H * sz.scale()).sum::<u32>()
