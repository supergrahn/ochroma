...
impl MotionFeature {
    ...
    pub fn distance(&self, other: &Self) -> f32 {
        ...
        let dhd = 1.0
            * ((self.heading_cos - other.heading_cos).powi(2)
...
mod tests {
    ...
    fn idle_query_matches_idle_clip() {
        ...
        assert_eq!(
            result.clip_name, "idle",
            "idle query should match idle clip, got '{}'",
            result.clip_name
        );
    }
    ...
    fn walk_query_matches_walk_clip() {
        ...
        let result = db.find_nearest(&query).unwrap();
        assert_eq!(
            result.clip_name, "walk_forward",
            "walk velocity query should match walk clip, got '{}'",
            result.clip_name
        );
    }
    ...
    fn sprint_query_matches_sprint_clip() {
        ...
        let result = db.find_nearest(&query).unwrap();
        assert_eq!(
            result.clip_name, "sprint",
            "sprint velocity query should match sprint clip, got '{}'",
            result.clip_name
        );
    }
    ...
    fn feature_count_matches_total_added() {
        ...
        assert_eq!(
            db.feature_count(),
            60 + 30 + 20,
            "total features: idle+walk+sprint"
        );
    }
}
