use vox_audio::{AudioCommand, SpectralReverb, SpectralSynth, synthesize_and_play};
...
fn glass_impact_produces_play_synth_command() {
    ...
    let nearby_splats: Vec<[u16; 16]> = (0..32).map(|_| [stone_reflectance_val; 16]).collect();
    ...
    match cmd {
        AudioCommand::PlaySynth { samples, volume } => {
            ...
            println!(
                "glass impact: samples.len={} peak={peak:.4} volume={volume}",
                samples.len()
            );
            ...
            assert!(
                peak > 0.01,
                "glass impact should produce audible signal, peak={peak}"
            );
        }
...
fn resonance_freq_of_glass_exceeds_stone() {
    ...
    assert!(
        glass_hz > stone_hz,
        "glass={glass_hz} Hz, stone={stone_hz} Hz"
    );
}
