...
impl SpectralAcousticProfile {
    ...
    pub fn from_spectral(bands_f16: &[u16; 16]) -> Self {
        const FREQ_MAP: [f32; 16] = [
            12000.0, 10000.0, 8000.0, 6500.0, 5000.0, 4000.0, 3000.0, 2200.0, 1500.0, 1100.0,
            800.0, 580.0, 400.0, 300.0, 250.0, 200.0,
        ];
        let bands: [f32; 16] =
            std::array::from_fn(|i| half::f16::from_bits(bands_f16[i]).to_f32().max(0.0));
        let sum: f32 = bands.iter().sum();
        let mean = sum / 16.0;

        let log_freq_sum: f32 = bands
            .iter()
            .zip(FREQ_MAP.iter())
            .map(|(&w, &f)| w * f.ln())
            .sum();
...
mod tests {
    ...
    fn metal_has_higher_q_than_soil() {
        ...
        assert!(
            metal_q > SpectralAcousticProfile::soil().q_factor,
    ...
    fn snow_has_longest_rt60() {
        ...
        println!(
            "snow rt60={}",
            SpectralAcousticProfile::snow().rt60_secs as u32
        );
        assert_eq!(
            max,
            SpectralAcousticProfile::snow().rt60_secs,
    ...
    fn glass_higher_resonance_than_soil() {
        assert!(
            SpectralAcousticProfile::glass().resonance_hz
    ...
    fn from_spectral_metal_like_profile() {
        ...
        assert!(
            profile.q_factor > 3.0,
            "flat high-reflectance should give Q > 3.0, got {}",
            profile.q_factor
        );
        assert!(
            profile.rt60_secs > 1.0,
            "flat high-reflectance should give RT60 > 1.0s, got {}",
            profile.rt60_secs
        );
    }
    ...
    fn from_spectral_dead_material() {
        ...
        let profile = SpectralAcousticProfile::from_spectral(&bands);
        assert!(
            profile.rt60_secs < 0.5,
            "dark material should give RT60 < 0.5s, got {}",
            profile.rt60_secs
        );
    }
}
