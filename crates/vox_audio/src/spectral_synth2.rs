...
const FREQ_MAP: [f32; 16] = [
    12000.0, 10000.0, 8000.0, 6500.0, 5000.0, 4000.0, 3000.0, 2200.0, 1500.0, 1100.0, 800.0, 580.0,
    400.0, 300.0, 250.0, 200.0,
];
    ...
    pub fn strike(spectral: &[u16; 16], impulse: f32) -> Vec<f32> {
        ...
        for harmonic in 0..HARMONICS {
            ...
            for (i, sample) in buf.iter_mut().enumerate() {
                ...
                *sample += amp * weight * envelope * (2.0 * std::f32::consts::PI * freq * t).sin();
            }
        }
...
mod tests {
    ...
    fn spectral_strike_centroid_glass_brighter_than_stone() {
        ...
        assert!(
            glass_c > stone_c + 1000.0,
            "glass centroid ({glass_c:.1} Hz) must exceed stone centroid ({stone_c:.1} Hz) by >1 kHz"
        );
        assert!(
            stone_c < 1500.0,
            "stone centroid should be low, got {stone_c:.1} Hz"
        );
        assert!(
            glass_c > 4000.0,
            "glass centroid should be high, got {glass_c:.1} Hz"
        );
    }

    #[test]
    fn blue_strike_sounds_different_from_red_strike() {
        let mut blue = [0u16; 16];
        blue[0] = half::f16::from_f32(1.0).to_bits();
        let mut red = [0u16; 16];
        red[15] = half::f16::from_f32(1.0).to_bits();
